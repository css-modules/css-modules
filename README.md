...
Note that when composing multiple classes from different files the order of application is *undefined*. Make sure to not define different values for the same property in multiple class names from different files when they are composed in a single class.
